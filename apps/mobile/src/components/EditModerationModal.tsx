...
export function EditModerationModal({ visible, onClose, onBack, onModerated }: { visible: boolean; onClose: () => void; onBack?: () => void; onModerated: () => void }) {
  ...
  return <Modal visible={visible} animationType="slide" onRequestClose={onBack ?? onClose}><SafeAreaView style={styles.root} edges={['top', 'bottom']}>
    ...
    {loading ? <View style={styles.center}><ActivityIndicator size="large" color={colors.brand} /><Text style={styles.muted}>Carregando correções…</Text></View>
      ...
      : <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled"><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.queue}>{items.map((item, index) => <Pressable key={item.id} onPress={() => { setSelectedId(item.id); setDecision(null); setReason(''); setMessage('') }} style={[styles.queueItem, selected?.id === item.id && styles.queueActive]}><Text style={[styles.queueText, selected?.id === item.id && styles.queueTextActive]}>{index + 1}. {item.stationName}</Text></Pressable>)}</ScrollView>{selected ? <>
        <View style={styles.card}><Text style={styles.cardEyebrow}>ALTERAÇÃO PROPOSTA</Text><Text style={styles.station}>{selected.stationName}</Text><Text style={styles.date}>Enviada em {new Date(selected.createdAt).toLocaleDateString('pt-BR')}</Text><Comparison label="NOME" before={selected.before.name} after={selected.after.name} styles={styles} /><Comparison label="BANDEIRA" before={selected.before.brand} after={selected.after.brand} styles={styles} /><Comparison label="RUA E NÚMERO" before={selected.before.address} after={selected.after.address} styles={styles} /><Comparison label="BAIRRO" before={selected.before.neighborhood} after={selected.after.neighborhood} styles={styles} /><Comparison label="CIDADE" before={selected.before.city} after={selected.after.city} styles={styles} /><Comparison label="UF" before={selected.before.state} after={selected.after.state} styles={styles} /><Comparison label="CEP" before={selected.before.postal_code} after={selected.after.postal_code} styles={styles} /><LocationComparison before={selected.before} after={selected.after} styles={styles} /><Comparison label="COMBUSTÍVEIS" before={formatList(selected.before.fuel_codes)} after={formatList(selected.after.fuel_codes)} styles={styles} /><Comparison label="SERVIÇOS" before={formatList(selected.before.service_codes)} after={formatList(selected.after.service_codes)} styles={styles} /></View>
...
function LocationComparison({ before, after, styles }: { before: PendingStationEdit['before']; after: PendingStationEdit['after']; styles: ReturnType<typeof createStyles> }) {
  if (before.latitude == null || before.longitude == null || after.latitude == null || after.longitude == null) return null
  const oldPoint = `${before.latitude.toFixed(5)}, ${before.longitude.toFixed(5)}`
  const newPoint = `${after.latitude.toFixed(5)}, ${after.longitude.toFixed(5)}`
  const distance = distanceMeters(before.latitude, before.longitude, after.latitude, after.longitude)
  return <View style={styles.comparison}><Text style={styles.label}>LOCALIZAÇÃO NO MAPA</Text><Text style={styles.before}>{oldPoint}</Text><View style={styles.arrowRow}><MaterialCommunityIcons name="arrow-down" size={15} color={distance > 1 ? styles.changed.color : styles.muted.color} /><Text style={[styles.after, distance > 1 && styles.changed]}>{newPoint}</Text></View><Text style={styles.distance}>{distance < 1 ? 'Ponto mantido' : `Ponto movido aproximadamente ${distance < 1000 ? `${Math.round(distance)} m` : `${(distance / 1000).toFixed(1)} km`}`}</Text></View>
}
