  ...
  return <Modal visible={visible} animationType="slide" onRequestClose={onClose}>
    <SafeAreaView style={styles.root} edges={['top', 'bottom']}>
      <View style={styles.header}>
        ...
        <Pressable accessibilityRole="button" accessibilityLabel="Fechar moderação" style={styles.close} onPress={onClose}><Text style={styles.closeText}>×</Text></Pressable>
      </View>
      {loading ? <View style={styles.center}><ActivityIndicator color={colors.brand} size="large" /><Text style={styles.muted}>Carregando cadastros…</Text></View> : loadError ? <View style={styles.errorState}><EmptyState icon="!" title="Não foi possível carregar" description={loadError} /><Pressable accessibilityRole="button" style={styles.retryButton} onPress={() => setReloadToken((value) => value + 1)}><Text style={styles.retryText}>Tentar novamente</Text></Pressable></View> : !stations.length ? <><EmptyState icon="✓" title="Tudo revisado" description="Não há postos aguardando moderação agora." />{message ? <Text accessibilityLiveRegion="polite" style={styles.successMessage}>{message}</Text> : null}</> : <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
...
const createStyles = (colors: ThemeColors) => StyleSheet.create({
  ...
  close: { marginLeft: 'auto', width: 44, height: 44, borderRadius: radius.full, backgroundColor: colors.surfaceAlt, alignItems: 'center', justifyContent: 'center' }, closeText: { color: colors.text, fontSize: 28, lineHeight: 30 },
